...
def get_data(data, frame_nos, dataset, topic, usernum, fps, milisec, width, height, view_width, view_height):
	VIEW_PATH = '../Viewport/'
	OBJ_PATH = '../Obj_traj/paris/'

	# obj_info = np.load(OBJ_PATH + 'ds{}/ds{}_topic{}.npy'.format(dataset, dataset, topic), allow_pickle=True,  encoding='latin1').item()
	# obj_info = np.load(OBJ_PATH + 'video1_obj_traj.npy'.format(dataset, dataset, topic), allow_pickle=True,  encoding='latin1').item()
	obj_info = np.load(OBJ_PATH + 'paris_obj_traj.npy'.format(dataset, dataset, topic), allow_pickle=True,  encoding='latin1').item()
