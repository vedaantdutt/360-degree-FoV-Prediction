...
def pred_frames(data, model, metric_X, metric_Y, frames, prev_frames, tile_manhattan_error, act_tiles, pred_tiles, count, width, height, nrow_tiles, ncol_tiles):
	...
	# plt.scatter(x_pred_list,y_pred_list)
	# plt.show()
...
def build_model(data, frame_nos, max_frame, tot_objects, width, height, nrow_tiles, ncol_tiles, fps, pred_nframe):
	...
	cnt = 0
	row = 1
	workbook = xlsxwriter.Workbook('paris_result.xlsx')
	worksheet = workbook.add_worksheet()
	worksheet.write(0,0,"Manhattan tile error")
	worksheet.write(0,1,"MAE X metric")
	worksheet.write(0,2,"MAE Y metric")
	...
	while True:
		...
		worksheet.write(row,col,(tile_manhattan_error*1.0 / count))
		print(metric_X, metric_Y)
		col = col + 1
		worksheet.write(row,col,metric_X.get())
		col = col + 1
		worksheet.write(row,col,metric_Y.get())
		print("\n")
		cnt = cnt + 1
		row = row + 1
		if cnt == 60:
